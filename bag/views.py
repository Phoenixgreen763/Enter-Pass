...
def view_bag(request):
    ...
    for item_id, quantity in bag.items():
        try:
            ...
            subtotal = event.price * quantity  # Calculate item subtotal
...
def add_to_bag(request, item_id):
    ...
    if request.method == "POST":
        ...
        print((
            f'Adding item ID {item_id} to the bag. '
            f'Quantity: {quantity}. '
            f'Current bag: {bag}'
        ))
        ...
        if str(item_id) in bag.keys():
            # Make sure to convert item_id to string
            bag[str(item_id)] += quantity
            messages.success(
                request,
                (
                    f'Updated {event.title} quantity to {bag[str(item_id)]}'
                )
            )
        else:
            bag[str(item_id)] = quantity
    ...
    if request.method == "POST":
        ...
        if quantity > 0:
            ...
            messages.success(
                request,
                (
                    f'Updated {event.title} quantity to '
                    f'{bag[str(item_id)]}'
                )
            )
        else:
            bag.pop(str(item_id), None)
            # Use None to avoid KeyError if not in bag
